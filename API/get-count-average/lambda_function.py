...
def build_query(filters):
    ...
    query = """
        SELECT 
            COUNT(DISTINCT sr.response_id) AS total_student_responses,
            COUNT(DISTINCT sr.email) AS total_students,
...
def lambda_handler(event, context):
    ...
    try:
        ...
        logger.info("Final query param: %s", query_params)
        ...
        return {
            ...
            'body': json.dumps({
                'logo' : logo,
                'total_student_responses': [str(result[0]),str(comparison[0])],
                'total_students': [str(result[1]),str(comparison[1])],
